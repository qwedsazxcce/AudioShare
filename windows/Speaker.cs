...
namespace AudioShare
{
    public class Speaker : INotifyPropertyChanged, IDisposable
    {
        ...
        public NamePair ChannelSelected
        {
            ...
            set
            {
                var target = NormalizeChannel(value.Key);
                _channel = target;
        ...
        {
            if (_channel == AudioChannel.None) return;
            ...
            try
            {
                ...
                if (tcpClient.Connected)
                {
                    ...
                    var preset = ChannelPresetCatalog.Get(_channel);
                    if (preset == null && _channel != AudioChannel.None)
                    {
                        throw new InvalidOperationException("Channel preset not found");
                    }
                    ...
                    var channelBytes = BitConverter.GetBytes(preset?.AndroidChannelMask ?? AndroidChannelMask.FrontLeft);
                    await WriteTcp(channelBytes);
                    await tcpClient.GetStream().ReadAsync(new byte[1], 0, 1);
                    _ = _dispatcher.InvokeAsync(() =>
                    {
                        AudioManager.StartCapture();
                        AudioManager.AudioAvailable += OnAudioAvailable;
        ...
        private async void OnAudioAvailable(object sender, AudioFrameEventArgs e)
        {
            lock (writeLock)
            {
                if (!Connected || _channel == AudioChannel.None || isBusy) return;
                isBusy = true;
            }
            var preset = ChannelPresetCatalog.Get(_channel);
            if (preset == null)
            ...
            try
            {
                byte[] payload = ChannelLayoutConverter.Convert(e, preset);
                if (payload == null || payload.Length == 0) return;
                if (!(await WriteTcp(payload, payload.Length, true)))
                {
        ...
        private AudioChannel NormalizeChannel(AudioChannel channel)
        {
            if (channel == AudioChannel.None) return channel;
            return ChannelPresetCatalog.Get(channel) != null ? channel : AudioChannel.Stereo;
        }

        private static void RebuildChannelNames()
        {
            void Refresh()
            {
                _channels.Clear();
                foreach (var preset in ChannelPresetCatalog.GetDisplayPairs())
                {
                    _channels.Add(new NamePair(preset.Key, preset.Value));
                }
                string disabled = Languages.Language.GetLanguageText("channelDisabled");
                if (string.IsNullOrWhiteSpace(disabled))
                {
                    disabled = "禁用";
                }
                _channels.Add(new NamePair(AudioChannel.None, disabled));
            }
        ...
        public static void RefreshChannelNames()
        {
            RebuildChannelNames();
        }
    }
