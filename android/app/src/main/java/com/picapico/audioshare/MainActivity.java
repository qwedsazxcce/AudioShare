...
public class MainActivity extends AppCompatActivity {
    ...
    private Spinner audioApiSpinner;
    private String[] audioApiValues;
    private boolean suppressAudioApiChange = false;
    private final TcpService.MessageListener messageListener = () -> {
        setConnectionStatus();
        setListenStatus();
        updateAudioApiSelection();
    };
    ...
    private final ServiceConnection connection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            setListenStatus();
            updateAudioApiSelection();
            audioApiSpinner.setEnabled(true);
        }

        @Override
        public void onServiceDisconnected(ComponentName arg0) {
            isBound = false;
            audioApiSpinner.setEnabled(false);
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        audioApiSpinner = findViewById(R.id.audioApiSpinner);
        audioApiValues = getResources().getStringArray(R.array.audio_api_values);
        ArrayAdapter<CharSequence> apiAdapter = ArrayAdapter.createFromResource(this,
                R.array.audio_api_entries, android.R.layout.simple_spinner_item);
        apiAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        audioApiSpinner.setAdapter(apiAdapter);
        audioApiSpinner.setEnabled(false);
        audioApiSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (suppressAudioApiChange || tcpService == null) return;
                if (position < 0 || position >= audioApiValues.length) return;
                AudioApi api = AudioApi.fromPreference(audioApiValues[position]);
                tcpService.setPreferredAudioApi(api);
            }
    ...
    private void updateAudioApiSelection(){
        if(tcpService == null || audioApiSpinner == null || audioApiValues == null) return;
        AudioApi api = tcpService.getAudioApi();
        int index = 0;
        for (int i = 0; i < audioApiValues.length; i++) {
            if (audioApiValues[i].equalsIgnoreCase(api.name())) {
                index = i;
                break;
            }
        }
        suppressAudioApiChange = true;
        audioApiSpinner.setSelection(index, false);
        suppressAudioApiChange = false;
    }
