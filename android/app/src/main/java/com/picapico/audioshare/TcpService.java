...
import com.picapico.audioshare.audio.AudioApi;
import com.picapico.audioshare.audio.AudioOutputEngine;
import com.picapico.audioshare.audio.AudioOutputFactory;
...
public class TcpService extends NotificationService {
    ...
    private static final int MAX_BUFFER_SIZE = 2 * 1024 * 1024; // 2MB maximum buffer size
    private final IBinder binder = new TcpBinder();
    private MessageListener mListener;
    private LocalServerSocket localServerSocket = null;
    private ServerSocket serverSocket = null;
    private AudioManager mAudioManager = null;
    private AudioOutputEngine mAudioEngine = null;
    private OutputStream mSocketOutputStream = null;
    private AudioApi mAudioApi = AudioApi.OPENSL;
    ...
    public void onCreate() {
        ...
        mAudioApi = resolveAudioApiPreference();
    ...
    private AudioApi resolveAudioApiPreference() {
        String value = mSharedPreferences.getString("audio-api", AudioApi.OPENSL.name());
        AudioApi api = AudioApi.fromPreference(value);
        if (api == AudioApi.AAUDIO && Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            return AudioApi.OPENSL;
        }
        return api;
    }
    ...
    public void setPreferredAudioApi(AudioApi api) {
        if (api == null) return;
        mSharedPreferences.edit().putString("audio-api", api.name()).apply();
        mAudioApi = resolveAudioApiPreference();
        if (mListener != null) {
            mListener.onMessage();
        }
    ...
    private void playAudio(int sampleRateInHz, int channelConfig, int audioEncoding, int bufferSizeInBytes, Closeable closer, InputStream inputStream, OutputStream outputStream){
        ...
        try {
            ...
            mAudioEngine = AudioOutputFactory.create(mAudioApi);
            boolean prepared = mAudioEngine.prepare(sampleRateInHz, channelCount, channelConfig, audioEncoding, bufferSizeInBytes);
            if (!prepared) {
                Log.w(TAG, "preferred audio api failed, fallback to AudioTrack");
                mAudioEngine.release();
                mAudioEngine = AudioOutputFactory.create(AudioApi.AUDIO_TRACK);
                prepared = mAudioEngine.prepare(sampleRateInHz, channelCount, channelConfig, audioEncoding, bufferSizeInBytes);
            }
            if (!prepared) {
                Log.e(TAG, "prepare audio engine failed");
                mAudioEngine.release();
                mAudioEngine = null;
            ...
            int dataLength;
            mAudioEngine.start();
            int sessionId = mAudioEngine.getAudioSessionId();
            if (sessionId > 0) {
                PlayerVisualizer.startBase(sessionId);
            } else {
            ...
            while (true) {
                try {
                    ...
                    if(dataLength > MAX_BUFFER_SIZE) {
                        Log.e(TAG, "buffer size exceeds limit: " + dataLength);
                        break;
                    }
                ...
                mExecutorService.execute(() -> {
                    int code = mAudioEngine.write(finalBuffer, 0, finalDataLength);
